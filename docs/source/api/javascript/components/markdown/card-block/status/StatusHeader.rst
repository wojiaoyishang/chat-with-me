...
.. CWM-AST-FUNCTION src/components/markdown/card-block/status/StatusHeader.jsx:379:10417:FUNCTION
...
根据执行分支返回结果；代表性返回表达式为 ``( <div className="flex items-center justify-between gap-2 group"> <div className={\`flex items-center gap-2.5 min-w-0 flex-1 ${shouldShowProgress ? 'overflow-visible' : 'overflow-h…``。
...
**主要协作调用**：``useRef``、``Boolean``、``useState``、``useEffect``、``visibleActions.map``。
...
.. CWM-AST-FUNCTION src/components/markdown/card-block/status/StatusHeader.jsx:3598:4587:FUNCTION
...
**性质**：同步局部函数；源码第 ``99``—``126`` 行；所属函数 ``memo callback @ 14``。
...
.. CWM-AST-FUNCTION src/components/markdown/card-block/status/StatusHeader.jsx:8995:9700:FUNCTION

.. rubric:: ``visibleActions.map callback @ 209``

.. code-block:: javascript

   visibleActions.map callback @ 209(action)

作为 ``visibleActions.map callback`` 集合回调，对当前元素执行映射、筛选、排序或归并。

**性质**：同步局部函数；源码第 ``209``—``219`` 行；所属函数 ``memo callback @ 14``。
...
.. CWM-AST-FUNCTION src/components/markdown/card-block/status/StatusHeader.jsx:9190:9233:FUNCTION

.. rubric:: ``onClick callback @ 213``

.. code-block:: javascript

   onClick callback @ 213(event)

处理 ``Click`` 用户交互或运行时事件。

**性质**：同步局部函数；源码第 ``213``—``213`` 行；所属函数 ``visibleActions.map callback @ 209``。
...
.. CWM-AST-FUNCTION src/components/markdown/card-block/status/StatusHeader.jsx:10418:11695:FUNCTION

.. rubric:: ``memo callback @ 241``

.. code-block:: javascript

   memo callback @ 241(prev, next)

实现 ``memo`` 对应的前端处理。

**性质**：同步局部函数；源码第 ``241``—``268`` 行。
