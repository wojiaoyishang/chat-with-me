...
根据执行分支返回结果；代表性返回表达式为 ``null``、``( <div className={\`flex items-center gap-0.5 text-sm transition-opacity duration-300 ${isRight ? 'justify-end' : 'justify-start'}\`}> <Button type="button" variant="ghost" size="ic…``。
