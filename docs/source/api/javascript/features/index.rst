.. title:: features JavaScript API

目录索引

本页列出该源码区域的全部 JavaScript/TypeScript 模块。模块内部标题不会展开到主导航。

.. list-table:: 模块清单
   :header-rows: 1
   :widths: 52 12 12 12 12

   * - 模块
     - 顶层
     - 方法
     - 局部/回调
     - 源码
   * - :doc:`src/features/chat/ChatPage </api/javascript/features/chat/ChatPage>`
     - 15
     - 0
     - 227
     - ``src/features/chat/ChatPage.jsx``
   * - :doc:`src/features/chat/attachmentVision </api/javascript/features/chat/attachmentVision>`
     - 6
     - 0
     - 1
     - ``src/features/chat/attachmentVision.js``
   * - :doc:`src/features/chat/composer/draftStore </api/javascript/features/chat/composer/draftStore>`
     - 17
     - 0
     - 4
     - ``src/features/chat/composer/draftStore.js``
   * - :doc:`src/features/chat/composer/messageDraftMount </api/javascript/features/chat/composer/messageDraftMount>`
     - 6
     - 0
     - 1
     - ``src/features/chat/composer/messageDraftMount.js``
   * - :doc:`src/features/chat/index </api/javascript/features/chat/index>`
     - 0
     - 0
     - 0
     - ``src/features/chat/index.js``
   * - :doc:`src/features/chat/modelCapabilities </api/javascript/features/chat/modelCapabilities>`
     - 3
     - 0
     - 0
     - ``src/features/chat/modelCapabilities.js``
   * - :doc:`src/features/chat/page/components/ChatHeader </api/javascript/features/chat/page/components/ChatHeader>`
     - 0
     - 0
     - 6
     - ``src/features/chat/page/components/ChatHeader.jsx``
   * - :doc:`src/features/chat/page/components/LoadingScreens </api/javascript/features/chat/page/components/LoadingScreens>`
     - 2
     - 0
     - 0
     - ``src/features/chat/page/components/LoadingScreens.jsx``
   * - :doc:`src/features/chat/page/components/MessageOverviewDialog </api/javascript/features/chat/page/components/MessageOverviewDialog>`
     - 0
     - 0
     - 10
     - ``src/features/chat/page/components/MessageOverviewDialog.jsx``
   * - :doc:`src/features/chat/page/components/MessageSummaryItem </api/javascript/features/chat/page/components/MessageSummaryItem>`
     - 0
     - 0
     - 3
     - ``src/features/chat/page/components/MessageSummaryItem.jsx``
   * - :doc:`src/features/chat/page/components/ModelItem </api/javascript/features/chat/page/components/ModelItem>`
     - 0
     - 0
     - 3
     - ``src/features/chat/page/components/ModelItem.jsx``
   * - :doc:`src/features/chat/page/components/ModelPreviewCard </api/javascript/features/chat/page/components/ModelPreviewCard>`
     - 0
     - 0
     - 3
     - ``src/features/chat/page/components/ModelPreviewCard.jsx``
   * - :doc:`src/features/chat/page/components/QuickUserMessageNavigator </api/javascript/features/chat/page/components/QuickUserMessageNavigator>`
     - 2
     - 0
     - 44
     - ``src/features/chat/page/components/QuickUserMessageNavigator.jsx``
   * - :doc:`src/features/chat/page/components/ResizeHandles </api/javascript/features/chat/page/components/ResizeHandles>`
     - 1
     - 0
     - 3
     - ``src/features/chat/page/components/ResizeHandles.jsx``
   * - :doc:`src/features/chat/page/components/RightSidebar </api/javascript/features/chat/page/components/RightSidebar>`
     - 2
     - 0
     - 21
     - ``src/features/chat/page/components/RightSidebar.jsx``
   * - :doc:`src/features/chat/page/components/RuntimeInspectorDialog </api/javascript/features/chat/page/components/RuntimeInspectorDialog>`
     - 11
     - 0
     - 47
     - ``src/features/chat/page/components/RuntimeInspectorDialog.jsx``
   * - :doc:`src/features/chat/page/components/ScrollToBottomButton </api/javascript/features/chat/page/components/ScrollToBottomButton>`
     - 0
     - 0
     - 3
     - ``src/features/chat/page/components/ScrollToBottomButton.jsx``
   * - :doc:`src/features/chat/page/components/SpeechPlayer </api/javascript/features/chat/page/components/SpeechPlayer>`
     - 21
     - 0
     - 134
     - ``src/features/chat/page/components/SpeechPlayer.jsx``
   * - :doc:`src/features/chat/page/components/SpeechSubtitleOverlay </api/javascript/features/chat/page/components/SpeechSubtitleOverlay>`
     - 5
     - 0
     - 27
     - ``src/features/chat/page/components/SpeechSubtitleOverlay.jsx``
   * - :doc:`src/features/chat/page/hooks/useChatScroll </api/javascript/features/chat/page/hooks/useChatScroll>`
     - 2
     - 0
     - 22
     - ``src/features/chat/page/hooks/useChatScroll.js``
   * - :doc:`src/features/chat/page/hooks/useChatSpeech </api/javascript/features/chat/page/hooks/useChatSpeech>`
     - 22
     - 0
     - 269
     - ``src/features/chat/page/hooks/useChatSpeech.js``
   * - :doc:`src/features/chat/page/hooks/useChatWindowMode </api/javascript/features/chat/page/hooks/useChatWindowMode>`
     - 1
     - 0
     - 23
     - ``src/features/chat/page/hooks/useChatWindowMode.js``
   * - :doc:`src/features/chat/page/hooks/useFileUpload </api/javascript/features/chat/page/hooks/useFileUpload>`
     - 1
     - 0
     - 42
     - ``src/features/chat/page/hooks/useFileUpload.js``
   * - :doc:`src/features/chat/page/utils/messageMountPoints </api/javascript/features/chat/page/utils/messageMountPoints>`
     - 1
     - 0
     - 3
     - ``src/features/chat/page/utils/messageMountPoints.js``
   * - :doc:`src/features/chat/page/utils/messageSummaries </api/javascript/features/chat/page/utils/messageSummaries>`
     - 4
     - 0
     - 4
     - ``src/features/chat/page/utils/messageSummaries.js``
   * - :doc:`src/features/chat/page/utils/networkMerge </api/javascript/features/chat/page/utils/networkMerge>`
     - 7
     - 0
     - 2
     - ``src/features/chat/page/utils/networkMerge.js``
   * - :doc:`src/features/chat/speech/backendAudio </api/javascript/features/chat/speech/backendAudio>`
     - 13
     - 0
     - 2
     - ``src/features/chat/speech/backendAudio.js``
   * - :doc:`src/features/chat/speech/constants </api/javascript/features/chat/speech/constants>`
     - 0
     - 0
     - 0
     - ``src/features/chat/speech/constants.js``
   * - :doc:`src/features/chat/speech/playbackTiming </api/javascript/features/chat/speech/playbackTiming>`
     - 0
     - 0
     - 0
     - ``src/features/chat/speech/playbackTiming.js``
   * - :doc:`src/features/chat/speech/speechRuntime </api/javascript/features/chat/speech/speechRuntime>`
     - 0
     - 0
     - 0
     - ``src/features/chat/speech/speechRuntime.js``
   * - :doc:`src/features/chat/speech/speechState </api/javascript/features/chat/speech/speechState>`
     - 2
     - 0
     - 0
     - ``src/features/chat/speech/speechState.js``
   * - :doc:`src/features/chat/speech/subtitleSettings </api/javascript/features/chat/speech/subtitleSettings>`
     - 10
     - 0
     - 1
     - ``src/features/chat/speech/subtitleSettings.js``
   * - :doc:`src/features/chat/speech/textMatching </api/javascript/features/chat/speech/textMatching>`
     - 10
     - 0
     - 1
     - ``src/features/chat/speech/textMatching.js``
   * - :doc:`src/features/chat/ui/AttachmentShowcase </api/javascript/features/chat/ui/AttachmentShowcase>`
     - 2
     - 0
     - 25
     - ``src/features/chat/ui/AttachmentShowcase.jsx``
   * - :doc:`src/features/chat/ui/ChatBox </api/javascript/features/chat/ui/ChatBox>`
     - 6
     - 0
     - 177
     - ``src/features/chat/ui/ChatBox.jsx``
   * - :doc:`src/features/chat/ui/ChatBoxHeader </api/javascript/features/chat/ui/ChatBoxHeader>`
     - 0
     - 0
     - 14
     - ``src/features/chat/ui/ChatBoxHeader.jsx``
   * - :doc:`src/features/chat/ui/ChatButton </api/javascript/features/chat/ui/ChatButton>`
     - 1
     - 0
     - 5
     - ``src/features/chat/ui/ChatButton.jsx``
   * - :doc:`src/features/chat/ui/DropFileLayer </api/javascript/features/chat/ui/DropFileLayer>`
     - 1
     - 0
     - 10
     - ``src/features/chat/ui/DropFileLayer.jsx``
   * - :doc:`src/features/chat/ui/FileUploadProgress </api/javascript/features/chat/ui/FileUploadProgress>`
     - 0
     - 0
     - 9
     - ``src/features/chat/ui/FileUploadProgress.jsx``
   * - :doc:`src/features/chat/ui/MessageContainer </api/javascript/features/chat/ui/MessageContainer>`
     - 0
     - 0
     - 7
     - ``src/features/chat/ui/MessageContainer.jsx``
   * - :doc:`src/features/chat/ui/QuickOptions </api/javascript/features/chat/ui/QuickOptions>`
     - 0
     - 0
     - 14
     - ``src/features/chat/ui/QuickOptions.jsx``
   * - :doc:`src/features/chat/ui/ToolButtons </api/javascript/features/chat/ui/ToolButtons>`
     - 6
     - 0
     - 36
     - ``src/features/chat/ui/ToolButtons.jsx``
   * - :doc:`src/features/chat/ui/chatbox/components/ChatBoxInteractionHost </api/javascript/features/chat/ui/chatbox/components/ChatBoxInteractionHost>`
     - 8
     - 0
     - 31
     - ``src/features/chat/ui/chatbox/components/ChatBoxInteractionHost.jsx``
   * - :doc:`src/features/chat/ui/chatbox/components/ComposerPrimaryAction </api/javascript/features/chat/ui/chatbox/components/ComposerPrimaryAction>`
     - 0
     - 0
     - 2
     - ``src/features/chat/ui/chatbox/components/ComposerPrimaryAction.jsx``
   * - :doc:`src/features/chat/ui/chatbox/components/EditMessageIndicator </api/javascript/features/chat/ui/chatbox/components/EditMessageIndicator>`
     - 0
     - 0
     - 4
     - ``src/features/chat/ui/chatbox/components/EditMessageIndicator.jsx``
   * - :doc:`src/features/chat/ui/chatbox/components/ExtraToolsMenuItems </api/javascript/features/chat/ui/chatbox/components/ExtraToolsMenuItems>`
     - 2
     - 0
     - 29
     - ``src/features/chat/ui/chatbox/components/ExtraToolsMenuItems.jsx``
   * - :doc:`src/features/chat/ui/chatbox/components/FullscreenEditorModal </api/javascript/features/chat/ui/chatbox/components/FullscreenEditorModal>`
     - 0
     - 0
     - 9
     - ``src/features/chat/ui/chatbox/components/FullscreenEditorModal.jsx``
   * - :doc:`src/features/chat/ui/chatbox/components/MessageInput </api/javascript/features/chat/ui/chatbox/components/MessageInput>`
     - 2
     - 0
     - 22
     - ``src/features/chat/ui/chatbox/components/MessageInput.jsx``
   * - :doc:`src/features/chat/ui/chatbox/components/RoleSelector </api/javascript/features/chat/ui/chatbox/components/RoleSelector>`
     - 1
     - 0
     - 5
     - ``src/features/chat/ui/chatbox/components/RoleSelector.jsx``
   * - :doc:`src/features/chat/ui/chatbox/components/SendButton </api/javascript/features/chat/ui/chatbox/components/SendButton>`
     - 0
     - 0
     - 3
     - ``src/features/chat/ui/chatbox/components/SendButton.jsx``
   * - :doc:`src/features/chat/ui/chatbox/components/VoiceInputButton </api/javascript/features/chat/ui/chatbox/components/VoiceInputButton>`
     - 0
     - 0
     - 1
     - ``src/features/chat/ui/chatbox/components/VoiceInputButton.jsx``
   * - :doc:`src/features/chat/ui/chatbox/components/VoicePermissionDialog </api/javascript/features/chat/ui/chatbox/components/VoicePermissionDialog>`
     - 0
     - 0
     - 2
     - ``src/features/chat/ui/chatbox/components/VoicePermissionDialog.jsx``
   * - :doc:`src/features/chat/ui/chatbox/utils/toolState </api/javascript/features/chat/ui/chatbox/utils/toolState>`
     - 7
     - 0
     - 3
     - ``src/features/chat/ui/chatbox/utils/toolState.js``
   * - :doc:`src/features/chat/ui/chatbox/utils/voiceRecorder </api/javascript/features/chat/ui/chatbox/utils/voiceRecorder>`
     - 15
     - 1
     - 34
     - ``src/features/chat/ui/chatbox/utils/voiceRecorder.js``
   * - :doc:`src/features/chat/ui/message/components/BranchMessageSelectorDialog </api/javascript/features/chat/ui/message/components/BranchMessageSelectorDialog>`
     - 0
     - 0
     - 11
     - ``src/features/chat/ui/message/components/BranchMessageSelectorDialog.jsx``
   * - :doc:`src/features/chat/ui/message/components/CompactedContextIndicator </api/javascript/features/chat/ui/message/components/CompactedContextIndicator>`
     - 0
     - 0
     - 1
     - ``src/features/chat/ui/message/components/CompactedContextIndicator.jsx``
   * - :doc:`src/features/chat/ui/message/components/ContextInspectorDialog </api/javascript/features/chat/ui/message/components/ContextInspectorDialog>`
     - 3
     - 0
     - 10
     - ``src/features/chat/ui/message/components/ContextInspectorDialog.jsx``
   * - :doc:`src/features/chat/ui/message/components/ContextStateIndicator </api/javascript/features/chat/ui/message/components/ContextStateIndicator>`
     - 0
     - 0
     - 2
     - ``src/features/chat/ui/message/components/ContextStateIndicator.jsx``
   * - :doc:`src/features/chat/ui/message/components/IgnoredContextIndicator </api/javascript/features/chat/ui/message/components/IgnoredContextIndicator>`
     - 0
     - 0
     - 1
     - ``src/features/chat/ui/message/components/IgnoredContextIndicator.jsx``
   * - :doc:`src/features/chat/ui/message/components/KnowledgeGraphViewer </api/javascript/features/chat/ui/message/components/KnowledgeGraphViewer>`
     - 1
     - 0
     - 34
     - ``src/features/chat/ui/message/components/KnowledgeGraphViewer.jsx``
   * - :doc:`src/features/chat/ui/message/components/LeftAvatarName </api/javascript/features/chat/ui/message/components/LeftAvatarName>`
     - 0
     - 0
     - 1
     - ``src/features/chat/ui/message/components/LeftAvatarName.jsx``
   * - :doc:`src/features/chat/ui/message/components/MessageActions </api/javascript/features/chat/ui/message/components/MessageActions>`
     - 0
     - 0
     - 3
     - ``src/features/chat/ui/message/components/MessageActions.jsx``
   * - :doc:`src/features/chat/ui/message/components/MessageAvatarMenu </api/javascript/features/chat/ui/message/components/MessageAvatarMenu>`
     - 1
     - 0
     - 8
     - ``src/features/chat/ui/message/components/MessageAvatarMenu.jsx``
   * - :doc:`src/features/chat/ui/message/components/MessageContextBadges </api/javascript/features/chat/ui/message/components/MessageContextBadges>`
     - 0
     - 0
     - 1
     - ``src/features/chat/ui/message/components/MessageContextBadges.jsx``
   * - :doc:`src/features/chat/ui/message/components/MessageItem </api/javascript/features/chat/ui/message/components/MessageItem>`
     - 0
     - 0
     - 25
     - ``src/features/chat/ui/message/components/MessageItem.jsx``
   * - :doc:`src/features/chat/ui/message/components/MessageMenuButton </api/javascript/features/chat/ui/message/components/MessageMenuButton>`
     - 0
     - 0
     - 7
     - ``src/features/chat/ui/message/components/MessageMenuButton.jsx``
   * - :doc:`src/features/chat/ui/message/components/MessagePaginator </api/javascript/features/chat/ui/message/components/MessagePaginator>`
     - 0
     - 0
     - 10
     - ``src/features/chat/ui/message/components/MessagePaginator.jsx``
   * - :doc:`src/features/chat/ui/message/components/MessageTools </api/javascript/features/chat/ui/message/components/MessageTools>`
     - 1
     - 0
     - 11
     - ``src/features/chat/ui/message/components/MessageTools.jsx``
   * - :doc:`src/features/chat/ui/message/components/ReplacementContextBadge </api/javascript/features/chat/ui/message/components/ReplacementContextBadge>`
     - 1
     - 0
     - 1
     - ``src/features/chat/ui/message/components/ReplacementContextBadge.jsx``
   * - :doc:`src/features/chat/ui/message/components/SpeechOverlayHighlighter </api/javascript/features/chat/ui/message/components/SpeechOverlayHighlighter>`
     - 37
     - 0
     - 36
     - ``src/features/chat/ui/message/components/SpeechOverlayHighlighter.jsx``
   * - :doc:`src/features/chat/ui/message/components/TextOnlyMessageContent </api/javascript/features/chat/ui/message/components/TextOnlyMessageContent>`
     - 0
     - 0
     - 3
     - ``src/features/chat/ui/message/components/TextOnlyMessageContent.jsx``
   * - :doc:`src/features/chat/ui/message/components/TooltipInfo </api/javascript/features/chat/ui/message/components/TooltipInfo>`
     - 5
     - 0
     - 11
     - ``src/features/chat/ui/message/components/TooltipInfo.jsx``
   * - :doc:`src/features/chat/ui/message/components/WidgetResponseMessage </api/javascript/features/chat/ui/message/components/WidgetResponseMessage>`
     - 4
     - 0
     - 4
     - ``src/features/chat/ui/message/components/WidgetResponseMessage.jsx``
   * - :doc:`src/features/chat/ui/message/hooks/useMessageAnimation </api/javascript/features/chat/ui/message/hooks/useMessageAnimation>`
     - 1
     - 0
     - 11
     - ``src/features/chat/ui/message/hooks/useMessageAnimation.js``
   * - :doc:`src/features/chat/ui/message/hooks/useMessageEvents </api/javascript/features/chat/ui/message/hooks/useMessageEvents>`
     - 1
     - 0
     - 3
     - ``src/features/chat/ui/message/hooks/useMessageEvents.js``
   * - :doc:`src/features/chat/ui/message/styles/messageAnimations </api/javascript/features/chat/ui/message/styles/messageAnimations>`
     - 0
     - 0
     - 0
     - ``src/features/chat/ui/message/styles/messageAnimations.js``
   * - :doc:`src/features/chat/ui/message/utils/copyContent </api/javascript/features/chat/ui/message/utils/copyContent>`
     - 3
     - 0
     - 0
     - ``src/features/chat/ui/message/utils/copyContent.js``
   * - :doc:`src/features/chat/ui/message/utils/messageActions </api/javascript/features/chat/ui/message/utils/messageActions>`
     - 1
     - 0
     - 3
     - ``src/features/chat/ui/message/utils/messageActions.js``
   * - :doc:`src/features/chat/ui/message/utils/speechContent </api/javascript/features/chat/ui/message/utils/speechContent>`
     - 22
     - 0
     - 6
     - ``src/features/chat/ui/message/utils/speechContent.js``
   * - :doc:`src/features/chat/voice/RealtimeVoiceButton </api/javascript/features/chat/voice/RealtimeVoiceButton>`
     - 1
     - 0
     - 0
     - ``src/features/chat/voice/RealtimeVoiceButton.jsx``
   * - :doc:`src/features/chat/voice/RealtimeVoiceSurface </api/javascript/features/chat/voice/RealtimeVoiceSurface>`
     - 8
     - 0
     - 6
     - ``src/features/chat/voice/RealtimeVoiceSurface.jsx``
   * - :doc:`src/features/chat/voice/index </api/javascript/features/chat/voice/index>`
     - 0
     - 0
     - 0
     - ``src/features/chat/voice/index.js``
   * - :doc:`src/features/chat/voice/useRealtimeVoiceConversation </api/javascript/features/chat/voice/useRealtimeVoiceConversation>`
     - 5
     - 0
     - 43
     - ``src/features/chat/voice/useRealtimeVoiceConversation.js``
   * - :doc:`src/features/chat/widgets/WidgetPresentationContext </api/javascript/features/chat/widgets/WidgetPresentationContext>`
     - 2
     - 0
     - 1
     - ``src/features/chat/widgets/WidgetPresentationContext.jsx``
   * - :doc:`src/features/notification/NotificationHost </api/javascript/features/notification/NotificationHost>`
     - 4
     - 0
     - 58
     - ``src/features/notification/NotificationHost.jsx``
   * - :doc:`src/features/notification/NotificationSettings </api/javascript/features/notification/NotificationSettings>`
     - 1
     - 0
     - 15
     - ``src/features/notification/NotificationSettings.jsx``
   * - :doc:`src/features/notification/NotificationToast </api/javascript/features/notification/NotificationToast>`
     - 1
     - 0
     - 0
     - ``src/features/notification/NotificationToast.jsx``
   * - :doc:`src/features/notification/notificationRegistry </api/javascript/features/notification/notificationRegistry>`
     - 4
     - 0
     - 4
     - ``src/features/notification/notificationRegistry.js``
   * - :doc:`src/features/notification/useNotificationStore </api/javascript/features/notification/useNotificationStore>`
     - 2
     - 0
     - 17
     - ``src/features/notification/useNotificationStore.js``
   * - :doc:`src/features/story/StoryCard </api/javascript/features/story/StoryCard>`
     - 1
     - 0
     - 13
     - ``src/features/story/StoryCard.jsx``
   * - :doc:`src/features/story/StoryReader </api/javascript/features/story/StoryReader>`
     - 5
     - 0
     - 38
     - ``src/features/story/StoryReader.jsx``
   * - :doc:`src/features/story/StorySelectorButton </api/javascript/features/story/StorySelectorButton>`
     - 1
     - 0
     - 15
     - ``src/features/story/StorySelectorButton.jsx``
   * - :doc:`src/features/tools/components/ConversationToolsDialog </api/javascript/features/tools/components/ConversationToolsDialog>`
     - 4
     - 0
     - 56
     - ``src/features/tools/components/ConversationToolsDialog.jsx``
   * - :doc:`src/features/workspace/WorkspaceSelector </api/javascript/features/workspace/WorkspaceSelector>`
     - 1
     - 0
     - 18
     - ``src/features/workspace/WorkspaceSelector.jsx``
   * - :doc:`src/features/workspace/WorkspaceSettingsDialog </api/javascript/features/workspace/WorkspaceSettingsDialog>`
     - 9
     - 0
     - 107
     - ``src/features/workspace/WorkspaceSettingsDialog.jsx``
   * - :doc:`src/features/workspace/components/FolderBrowser </api/javascript/features/workspace/components/FolderBrowser>`
     - 2
     - 0
     - 17
     - ``src/features/workspace/components/FolderBrowser.jsx``
   * - :doc:`src/features/workspace/components/FolderBrowserDialog </api/javascript/features/workspace/components/FolderBrowserDialog>`
     - 1
     - 0
     - 1
     - ``src/features/workspace/components/FolderBrowserDialog.jsx``
   * - :doc:`src/features/workspace/useWorkspaceTransferStore </api/javascript/features/workspace/useWorkspaceTransferStore>`
     - 5
     - 0
     - 5
     - ``src/features/workspace/useWorkspaceTransferStore.js``

.. toctree::
   :hidden:
   :maxdepth: 1

   /api/javascript/features/chat/ChatPage
   /api/javascript/features/chat/attachmentVision
   /api/javascript/features/chat/composer/draftStore
   /api/javascript/features/chat/composer/messageDraftMount
   /api/javascript/features/chat/index
   /api/javascript/features/chat/modelCapabilities
   /api/javascript/features/chat/page/components/ChatHeader
   /api/javascript/features/chat/page/components/LoadingScreens
   /api/javascript/features/chat/page/components/MessageOverviewDialog
   /api/javascript/features/chat/page/components/MessageSummaryItem
   /api/javascript/features/chat/page/components/ModelItem
   /api/javascript/features/chat/page/components/ModelPreviewCard
   /api/javascript/features/chat/page/components/QuickUserMessageNavigator
   /api/javascript/features/chat/page/components/ResizeHandles
   /api/javascript/features/chat/page/components/RightSidebar
   /api/javascript/features/chat/page/components/RuntimeInspectorDialog
   /api/javascript/features/chat/page/components/ScrollToBottomButton
   /api/javascript/features/chat/page/components/SpeechPlayer
   /api/javascript/features/chat/page/components/SpeechSubtitleOverlay
   /api/javascript/features/chat/page/hooks/useChatScroll
   /api/javascript/features/chat/page/hooks/useChatSpeech
   /api/javascript/features/chat/page/hooks/useChatWindowMode
   /api/javascript/features/chat/page/hooks/useFileUpload
   /api/javascript/features/chat/page/utils/messageMountPoints
   /api/javascript/features/chat/page/utils/messageSummaries
   /api/javascript/features/chat/page/utils/networkMerge
   /api/javascript/features/chat/speech/backendAudio
   /api/javascript/features/chat/speech/constants
   /api/javascript/features/chat/speech/playbackTiming
   /api/javascript/features/chat/speech/speechRuntime
   /api/javascript/features/chat/speech/speechState
   /api/javascript/features/chat/speech/subtitleSettings
   /api/javascript/features/chat/speech/textMatching
   /api/javascript/features/chat/ui/AttachmentShowcase
   /api/javascript/features/chat/ui/ChatBox
   /api/javascript/features/chat/ui/ChatBoxHeader
   /api/javascript/features/chat/ui/ChatButton
   /api/javascript/features/chat/ui/DropFileLayer
   /api/javascript/features/chat/ui/FileUploadProgress
   /api/javascript/features/chat/ui/MessageContainer
   /api/javascript/features/chat/ui/QuickOptions
   /api/javascript/features/chat/ui/ToolButtons
   
   /api/javascript/features/chat/ui/chatbox/utils/toolState
   
   /api/javascript/features/chat/ui/message/utils/copyContent
   /api/javascript/features/chat/ui/message/utils/messageActions
   /api/javascript/features/chat/ui/message/utils/speechContent
   /api/javascript/features/chat/voice/RealtimeVoiceButton
   /api/javascript/features/chat/voice/RealtimeVoiceSurface
   /api/javascript/features/chat/voice/index
   /api/javascript/features/chat/voice/useRealtimeVoiceConversation
   /api/javascript/features/chat/widgets/WidgetPresentationContext
   /api/javascript/features/notification/NotificationHost
   /api/javascript/features/notification/NotificationSettings
   /api/javascript/features/notification/NotificationToast
   /api/javascript/features/notification/notificationRegistry
   /api/javascript/features/notification/useNotificationStore
   /api/javascript/features/story/StoryCard
   /api/javascript/features/story/StoryReader
   /api/javascript/features/story/StorySelectorButton
   /api/javascript/features/tools/components/ConversationToolsDialog
   /api/javascript/features/workspace/WorkspaceSelector
   /api/javascript/features/workspace/WorkspaceSettingsDialog
   /api/javascript/features/workspace/components/FolderBrowser
   /api/javascript/features/workspace/components/FolderBrowserDialog
   /api/javascript/features/workspace/useWorkspaceTransferStore

