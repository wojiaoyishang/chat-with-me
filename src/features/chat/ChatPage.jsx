...
import {
    ...
    getSpeechTagScore,
} from '@/features/chat/speech/speechRuntime.js';
...
function ChatPage({
    ...
    // TTS 高亮同步：Audio.onplaying 在部分浏览器里会早于用户真正听到声音，
    // 因此高亮不要在 onplaying 立即切换，而是在音频 currentTime 开始推进后再切换。
    const TTS_HIGHLIGHT_START_DELAY_MS = 500;
    const TTS_HIGHLIGHT_MIN_CURRENT_TIME = 0.035;
    const TTS_NEXT_SEGMENT_TAIL_DELAY_MS = 1000;
...
// ========= 上传相关 =========
    ...
    const playNextBackendSpeechSegment = useCallback(() => {
        ...
        const applyPlaybackSegmentWhenAudible = (source = 'unknown') => {
            ...
            // 对极短音频，如果等不到稳定的 currentTime，也不要永久不高亮；但仍然避开 onplaying 的同一瞬间。
            if (!hasAudibleProgress && hasDuration && audio.duration > 0.18) {
                return;
            }
        ...
        const schedulePlaybackSegmentHighlight = () => {
            ...
            highlightTimer = window.setTimeout(() => {
                highlightTimer = null;
                applyPlaybackSegmentWhenAudible('onplaying-delay');
            }, TTS_HIGHLIGHT_START_DELAY_MS);
        };

        audio.onplaying = () => {
            if (isStalePlayback()) return;
            // onplaying 只能说明浏览器进入播放状态，不代表用户已经听到当前句。
            // 高亮延迟到 currentTime 推进后再切换，避免“文字抢跑”。
