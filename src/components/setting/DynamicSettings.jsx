        ...
        return (
            <SettingRow text={item.text} tips={item.tips} expanded={false}>
                <div className="flex items-center gap-2.5 flex-1 justify-end">
                    <div
                        className="flex items-center border border-[#e1e4e8] dark:border-[#3a3f45] rounded-md overflow-hidden flex-shrink-0">
                        <button
        ...
        return (
            <SettingRow text={item.text} tips={item.tips}>
                <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                    ...
                    <DialogContent
                        className="w-[min(90vw,520px)] z-999 max-w-none rounded-3xl border-[#e1e4e8] dark:border-[#3a3f45] bg-white dark:bg-[#1c1e21] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.3)]">
                        <DialogHeader>
...
function RadioItem({item, path, groupPath}) {
    ...
    return (
        <SettingRow text={item.text} tips={item.tips}>
            ...
            >
                <span
                    className={`w-2 h-2 rounded-full bg-[#2563eb] dark:bg-[#3b82f6] transition-all ${isSelected ? "scale-100" : "scale-0"}`}/>
            </button>
        </SettingRow>
    );
}
...
export default function DynamicSettings({
    ...
    const update = useCallback((path, value) => {
        setValues((prev) => {
            ...
            // 异步调用避免在 render 中触发 side effect
    ...
    useEffect(() => {
        // 使用 setTimeout 保持与 update 一致的异步行为
        setTimeout(() => {
    ...
    for (const item of config) {
        ...
        if (item.type === "group" && item.name && item.children) {
            ...
            if (hasRadios) {
                ...
                // 如果 initialValues 中有该 group 的值且是字符串，直接使用
                ...
                for (const child of item.children) {
                    if (child.name) {
                        // 优先使用 initialValues 中的值，否则使用 default
