...
显示在输入框左侧的常驻工具按钮，**推荐不超过 3 个**（超过 3 个可能导致排版错乱）。
...
{
  "type": "toggle",  // 菜单类型
  "name": "autoTranslate",  // 唯一标识（非 label/separator 类型必需）
  "text": "自动翻译",  // 显示文本
  "iconType": "library",  // 图标类型（可选）
  "iconData": "earth"  // 图标数据（可选）
}
```
