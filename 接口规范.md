...
- type：事件的类型(string)，分为消息事件 `message` 、 控件事件 `widget` 、 页面事件 `page` 和 Websocket 事件 `websocket`
- target：接受事件的对象(string)，具体查看说明
- payload：对象调整的参数(object)
- markId：该事件的标记，用于区分不同场景而标记，若留空或null则直接在当前页面上执行，如果不提供 markId 默认是在所有页面上广播的
...
如果没有 markId 就向服务器请求一个，请求时随机生成一个唯一uuid字符串（markId）并发送给服务器，负载如下：
...
```javascript
{
    ...
    markId: "3e4f8a1b-5c9d-4e2a-8f1a-7b3c9d2e4f5a",  // 本地随机字串
    isReply: false
}
```
...
```javascript
{
    ...
    payload: {
        command: "Get-MarkId", 
        markId: "566f8a77-3c9d-112a-8b1a-2453c92e434b" // 服务器正式字串
    }, 
    markId: "3e4f8a1b-5c9d-4e2a-8f1a-7b3c9d2e4f5a",
    isReply: true
}
```
...
如果是 PUT 请求，则代表前端切换了消息的分支选项，如果是请求缺失的消息不会触发这个响应，表单内容：
...
- SyncMessageSwitch 布尔值，是否实时同步消息选择分支给前端，如果为真每次切换分支时都会 put 一个请求给服务器